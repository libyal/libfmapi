...
LIBRARY_TESTS="entry_identifier error one_off_entry_identifier property_type support value_type x400_object_identifier";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fmapi_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfmapi" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
